...
function declareImports(ast: AST): string {
    function generateImport(ast: AST): string[] {
        let content = ['import * as m from "django-client-framework"']
        switch (ast.type) {
            case "INTERFACE":
                for (const prop of ast.rels) {
                    content = content.concat(declareImports(prop))
                }
                return content
            case "RELATIONAL_PROPERTY":
                return [`import { ${ast.to} } from "../${ast.to}"`]
            default:
                return content
        }
    }
    return Array.from(new Set(generateImport(ast))).join("\n")
}
        ...
        case "TUPLE":
            return (() => {
                ...
                function addSpreadParam(params: string[]): string[] {
                    if (spreadParam) {
                        const spread =
                            "...(" + generateType(spreadParam, options) + ")[]"
                        params.push(spread)
                    }
                    return params
                }
...
function generateInterface(ast: TInterface, options: Options): string {
    return (
        ...
        ast.params
            ...
            .join("\n") +
        "\n" +
        ast.rels.map(generateRelationProperty).join("\n") +
        "}"
    )
}

function generateRelationProperty(rel: TRelationalProperty) {
    switch (rel.relation) {
        case "ForeignKey":
        case "OneToOneRel": {
            return `get ${rel.keyName}() { return new m.RelatedObjectManager(${rel.to}, this, "${rel.keyName}") }`
        }
        case "ManyToOneRel":
        case "ManyToManyRel": {
            return `get ${rel.keyName}() { return new m.RelatedCollectionManager(${rel.to}, this, "${rel.keyName}") }`
        }
    }
}
...
function generateStandaloneEnum(ast: TEnum, options: Options): string {
    return (
        ...
        "\n" +
        ast.params
            .map(({ ast, keyName }) => keyName + " = " + generateType(ast, options))
            .join(",\n") +
        "\n" +
        "}"
    )
...
function generateStandaloneInterface(ast: TNamedInterface, options: Options): string {
    ...
    return (
        (hasComment(ast) ? generateComment(ast.comment) + "\n" : "") +
        `export class ${toSafeString(ast.standaloneName)} extends m.Model ` +
        generateInterface(ast, options)
    )
}
