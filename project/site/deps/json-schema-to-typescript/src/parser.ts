...
export function parse(
    ...
    ast.params = types.map((type) =>
        ...
        parseAsTypeWithCache(
            omit(schema, "description", "id", "title"),
            type,
            options,
            keyName,
            processed,
...
function parseNonLiteral(
    ...
    switch (type) {
        case "ALL_OF":
            return {
                ...
                params: schema.allOf!.map((_) =>
                    parse(_, options, undefined, processed, usedNames)
                ),
                type: "INTERSECTION",
            }
        case "ANY":
        ...
        case "REFERENCE":
            throw Error(
                format("Refs should have been resolved by the resolver!", schema)
            )
        case "STRING":
            return {
                comment: schema.description,
...
function newInterface(
    ...
    return {
        comment: schema.description,
        keyName,
        params: parseInterfaceParams(schema, options, processed, usedNames, name),
        rels: parseInterfaceRels(schema),
        standaloneName: name,
...
function parseInterfaceRels(schema: SchemaSchema): TRelationalProperty[] {
    if (schema.relationalProperties) {
        return map(schema.relationalProperties, (value, key: string) => ({
